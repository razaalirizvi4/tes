    ...
    try {
      ...
      const restaurant = await prisma.restaurant.create({
        data: {
          ...restaurantInfo,
          vendor: {
            connect: { id: restaurantData.vendor.id },
          },
          menuItems: {
            create: menuItems,
          },
      ...
      // Seed default addon groups for this restaurant
      ...
      const toppingsGroup = await prisma.addonGroup.create({
        data: {
          restaurantId: restaurant.id,
          name: 'Extra Toppings',
          displayName: 'Select Extra Toppings',
          ...
          options: {
            create: [
              { name: 'Extra Parmesan Cheese', priceAdjustment: 1.50, sortOrder: 0 },
              { name: 'Grilled Chicken Breast', priceAdjustment: 4.00, sortOrder: 1 },
      ...
      // Assign all addon groups to all menu items of this restaurant
      for (const item of restaurant.menuItems) {
        await prisma.menuItemAddon.createMany({
          data: [
            ...
            { menuItemId: item.id, addonGroupId: toppingsGroup.id, sortOrder: 2 },
          ],
        });
      }
      console.log(`  ? Seeded addon groups for '${restaurant.name}' (${restaurant.menuItems.length} items)`);
    } catch (error) {
